<%-- 
    Document   : editbill
    Created on : Dec 19, 2021, 1:15:34 PM
    Author     : Quan
--%>
                ...
                <div id="layoutSidenav_content">
                    <main>
                        ...
                        <c:if test="${not empty requestScope.bill}">
                            ...
                            <form action="${pageContext.request.contextPath}/AdminBill" method="post">
                                <input type="hidden" name="action" value="3"/>
                                <input type="hidden" name="id" value="${bill.billId}"/>
                                Staff: ${bill.staff.fullName}<br/>
                                Status: 
                                <select name="statusid" class="form-select-sm">
                                    <c:forEach var="s" items="${lstStatus}">
                                        <option value="${s.statusId}" ${s.statusId==bill.statusId?"selected='selected'":""}>
                                            ${s.statusName}
                                        </option>
                                    </c:forEach>
                                </select>
                                <input type="submit" value="Update" class="btn-primary"/> <br/>
