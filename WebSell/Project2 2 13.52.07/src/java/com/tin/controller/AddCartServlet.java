...
public class AddCartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getSession().getAttribute("listItem")==null){
            //cart is empty
            ArrayList<CartItem> lst = new ArrayList<>();
            CartItem item = new CartItem();
            item.setQuantity(1);
            item.setProduct(proDAO.getById(id));
            lst.add(item);
            request.getSession().setAttribute("listItem", lst);
        } else{
            //If exist, check item in cart
            boolean flag=false;
            ...
            if(flag==false){
                ...
                c.setQuantity(1);
                c.setProduct(proDAO.getById(id));
                lst.add(c);
            }
            //list updated
            request.getSession().setAttribute("listItem", lst);
